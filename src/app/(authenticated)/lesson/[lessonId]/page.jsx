...
export default async function LessonPage({ params }) {
  ...
  // Fetch resources and note in parallel — each has its own error boundary
  const [resourcesResult, noteResult] = await Promise.allSettled([
    user?.id ? getLessonResources(user.id, lessonId) : Promise.resolve({ aiResources: [], userResources: [] }),
    user?.id ? getLessonNote(user.id, lessonId) : Promise.resolve(null),
  ]);

  // Serialize MongoDB ObjectIds/Dates for client component props
  const aiResources =
  ...
  return (
    <PageWrapper>
      ...
      <section className="mt-6 max-w-4xl">
        ...
        <h1 className="mt-2 text-3xl font-semibold tracking-tight sm:text-4xl">{lesson.title}</h1>
        <p className="mt-4 text-lg leading-8 text-[var(--muted-foreground)]">{lesson.description}</p>
